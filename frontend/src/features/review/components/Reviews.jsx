...
export const Reviews = ({productId,averageRating}) => {
    ...
    useEffect(()=>{
        ...
        if(reviewAddStatus==='fulfilled'){
            ...
            toast.error("Lỗi khi đăng đánh giá, vui lòng thử lại sau")
        }
        ...
        <Stack rowGap={5} alignSelf={"flex-start"}  width={is480?"90vw":is840?"25rem":'40rem'}>
                    ...
                    reviews?.length?(
                        <Stack rowGap={3}>
                            ...
                            <Stack rowGap={1} >
                                ...
                                <Typography variant='h6' color={'text.secondary'} >Có {reviews.length} {reviews.length===1?"Review":"Reviews"}</Typography>
                            </Stack>
            ...
            {   
                ...
                <Stack rowGap={3} position={'relative'} component={'form'} noValidate onSubmit={handleSubmit(handleAddReview)}>
                    ...
                    <TextField id='reviewTextFeild' {...register("comment",{required:true})} sx={{mt:4,width:is840?"100%":"40rem"}}  multiline rows={6} fullWidth placeholder='Hãy đánh giá tui...'/>
                    
                    <Stack>
                        <Typography gutterBottom variant='body2'> Bạn đánh giá mấy sao cho tui?</Typography>
