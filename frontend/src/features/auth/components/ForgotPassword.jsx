        ...
        <Stack rowGap={'1rem'}>
            <Stack component={Paper} elevation={2}>
                <Stack component={'form'} width={is500?"95vw":'30rem'} p={is500?"1rem":'1.5rem'} rowGap={'1rem'} noValidate onSubmit={handleSubmit(handleForgotPassword)}>
                        ...
                        <Stack rowGap={'.4rem'}>
                            <Typography variant='h5' fontWeight={600}>{status==='fullfilled'?"Đã gửi đến email của bạn":"Quên mật khẩu ư?"}</Typography>
                            <Typography color={'text.secondary'} variant='body2'>{status==='fullfilled'?"Check hộp thư đến email của bạn để reset mật khẩu":"Nhập email đăng kí của bạn để nhận thư reset password"}</Typography>
                        </Stack>
                        ...
                        <motion.div whileHover={{y:-2}}>
                            <TextField fullWidth sx={{mt:1}} {...register("email",{required:"Bắt buộc",pattern:{value:/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/g,message:"Hãy nhập email khả dụng"}})} placeholder='Enter email'/>
                            {errors.email && <FormHelperText sx={{fontSize:".9rem",mt:1}} error >{errors.email.message}</FormHelperText>}
                        </motion.div>

                        <motion.div whileHover={{scale:1.020}} whileTap={{scale:1}}>
                            <LoadingButton sx={{height:'2.5rem'}} fullWidth loading={status==='pending'} type='submit' variant='contained'>Gửi</LoadingButton>
                        </motion.div>
                        </>
                        }
                </Stack>
